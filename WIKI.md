...
A handful of Gabriel Liu's early commits (e.g. the SonarCloud/Snyk CI fixes and the workflow-mistake note above) show up on GitHub as authored by a different, unrelated personal account (`hliu673`) rather than his `Gabeliu` account. This happened because his local git config was set to an email verified under that other account. That email is still needed for another project, so it can't be moved over to relink the old commits without a disruptive history rewrite of `main`; the local git config has been corrected, so this does not affect any commits going forward.
